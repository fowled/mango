...
module.exports = {
    ...
    async execute(Client: Discord.Client, member: Discord.GuildMember) {
        ...
        if (fetchUserFromDB.data) {
            await supabase.from("users").update({ guilds: [...fetchUserFromDB.data.guilds, member.guild.id] }).like("user_id", member.user.id);
        }
        ...
        try {
            ...
            warn("Didn't find the channel to post attachment [guildMemberAdd]");
        }
    },
};
