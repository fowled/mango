...
module.exports = {
	...
	async execute(interaction: Discord.CommandInteraction & Discord.Message, Client: Discord.Client) {
		...
		const commandEmbed: Discord.MessageEmbed = new Discord.MessageEmbed().setDescription(`**${interaction.user.tag}** just ran the \`${interaction.commandName}\` command in *${interaction.guild.name}*.`).setAuthor(interaction.user.username, interaction.user.displayAvatarURL()).setColor("NOT_QUITE_BLACK").setTimestamp().setFooter(Client.user.username, Client.user.displayAvatarURL());
