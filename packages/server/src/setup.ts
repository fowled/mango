...
async function startPrompt() {
    ...
    const content = [`TOKEN:${prompt.token}`, `API_KEY:${prompt.api_key}`, `CMD_CHANNEL:${prompt.cmd_channel}`, `ERR_CHANNEL:${prompt.err_channel}`, `CMD_CHANNEL:${prompt.cmd_channel}`, `GUILD_ID:${prompt.guild_id}`].join('\n');
