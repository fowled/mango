...
module.exports = {
	...
	async execute(Client: Discord.Client, interaction: Discord.CommandInteraction, args: string[]) {
		...
		insertLog(Client, interaction.guild.id, interaction.user, `**${interaction.channel}** (\`${(interaction.channel as Discord.TextChannel).name}\`) has been locked by *${interaction.user.tag}*`);
	},
};
