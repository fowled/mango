            ...
            msg
                ...
                .then(async (collected) => {
                    ...
                    if (isCaseOccupied(emojiToLetter(collected.first().emoji.name))) {
                        const status = new Discord.EmbedBuilder().setAuthor({
                            name: collected.first().users.cache.last().tag,
                            iconURL: collected.first().users.cache.last().avatarURL()
                        }).setColor("#1E90FF").setDescription(`**${collected.first().users.cache.last().tag}** tried to react with the ${collected.first().emoji.name} emoji, but this case is already occupied by a player...`);
                    ...
                    const status = new Discord.EmbedBuilder().setAuthor({
                        name: collected.first().users.cache.last().tag,
                        iconURL: collected.first().users.cache.last().avatarURL()
                    }).setColor("#1E90FF").setDescription(`**${collected.first().users.cache.last().tag}** reacted with the ${collected.first().emoji.name} emoji.`);
                    ...
                    if (checkIfWin(turn)) {
                        const status = new Discord.EmbedBuilder().setAuthor({
                            name: collected.first().users.cache.last().tag,
                            iconURL: collected.first().users.cache.last().avatarURL()
                        }).setColor("#ffff00").setDescription(`**${collected.first().users.cache.last().tag}** won the game. GG!`);
                        ...
                        const status = new Discord.EmbedBuilder().setAuthor({
                            name: collected.first().users.cache.last().tag,
                            iconURL: collected.first().users.cache.last().avatarURL()
                        }).setColor("#1E90FF").setDescription(":crossed_swords: Nobody won... That's a draw!");
