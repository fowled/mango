...
module.exports = {
    ...
    async execute(Client: Discord.Client) {
        ...
        const statuses = [
            ...
            {name: `${Client.guilds.cache.size.toString()} guilds`, type: Discord.ActivityType.Competing}
        ]

        function switchStatuses() {
            Client.user.setActivity(statuses[currentStatus].name, {type: statuses[currentStatus].type as Discord.ActivityType.Competing | Discord.ActivityType.Watching});
